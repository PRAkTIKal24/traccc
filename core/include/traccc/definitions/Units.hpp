...
/// in  ``traccc::UnitConstants``. Multiplying a value with the unit constant
/// produces the equivalent value in the native unit, e.g.
///
/// .. code-block:: cpp
///
///    double length = 1 * traccc::UnitConstants::m;       // length == 1000.0
///    double momentum = 100 * traccc::UnitConstants::MeV; // momentum == 0.1
///
...
///    double lengthInM = length / traccc::UnitConstants::m; // == 0.01;
///
...
/// ``traccc::UnitLiterals``. This allows us to express quantities in a concise
/// way:
///
/// .. code-block:: cpp
///
///    using namespace traccc::UnitLiterals;
///
...
///    ``traccc::UnitLiterals`` it should not be used in headers since it would
...
///    double width    = 12 * traccc::UnitConstants::mm;
///    double mmuon    = 105.7 * traccc::UnitConstants::MeV;
...
///    double t_in_ns    = trackPars.time() / traccc::UnitConstants::ns;
...
#define ACTS_DEFINE_UNIT_LITERAL(name)                        \
  constexpr double operator"" _##name(long double x) {        \
    return ::traccc::UnitConstants::name * x;                   \
  }                                                           \
