...
/// in `namespace traccc` and are prefixed to avoid naming collisions.
...
using BoundVector = TracccVector<eBoundSize>;
using BoundMatrix = TracccMatrix<eBoundSize, eBoundSize>;
using BoundSquareMatrix = TracccSquareMatrix<eBoundSize>;
// Mapping from bound track parameters.
using BoundToFreeMatrix = TracccMatrix<eFreeSize, eBoundSize>;

// Shorthand vector/matrix types related to free track parameters.
using FreeVector = TracccVector<eFreeSize>;
using FreeMatrix = TracccMatrix<eFreeSize, eFreeSize>;
using FreeSquareMatrix = TracccSquareMatrix<eFreeSize>;
// Mapping from free track parameters.
using FreeToBoundMatrix = TracccMatrix<eBoundSize, eFreeSize>;
using FreeToPathMatrix = TracccMatrix<1, eFreeSize>;
