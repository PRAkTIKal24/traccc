...
#define TRACCC_CHECK_BIT(value, mask) ((value & mask) == mask)
...
template <typename T>
std::tuple<typename T::value_type, TracccScalar> range_medium(const T& tseries) {
  auto [min, max] = min_max(tseries);
  typename T::value_type range = (max - min);
  TracccScalar medium = static_cast<TracccScalar>((max + min) * 0.5);
