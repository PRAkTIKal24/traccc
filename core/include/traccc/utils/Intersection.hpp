...
class Intersection {
 public:
  /// Position type
  using Position = TracccVector<DIM>;
  /// Status enum
  ...
  constexpr TracccScalar pathLength() const { return m_pathLength; }
 ...
 private:
  ...
  TracccScalar m_pathLength = std::numeric_limits<double>::infinity();
...
class ObjectIntersection {
 public:
  ...
  constexpr TracccScalar pathLength() const {
...
bool checkIntersection(const intersection_t& intersection, double pLimit,
  ...
  TRACCC_VERBOSE(" -> pLimit, oLimit, cLimit: " << pLimit << ", " << oLimit
                                              << ", " << cLimit);
  ...
  if (accept) {
    TRACCC_VERBOSE("Intersection is WITHIN limit");
  } else {
    TRACCC_VERBOSE("Intersection is OUTSIDE limit because: ");
    if (not coCriterion) {
      TRACCC_VERBOSE("- intersection path length "
