...
#include "traccc/definitions/CommonActs.hpp"
...
inline std::array<TracccScalar, 5> evaluateTrigonomics(const Vector3& direction) {
  const TracccScalar x = direction(0);  // == cos(phi) * sin(theta)
  const TracccScalar y = direction(1);  // == sin(phi) * sin(theta)
  const TracccScalar z = direction(2);  // == cos(theta)
  // can be turned into cosine/sine
  const TracccScalar cosTheta = z;
  const TracccScalar sinTheta = std::hypot(x, y);
  const TracccScalar invSinTheta = 1. / sinTheta;
  const TracccScalar cosPhi = x * invSinTheta;
  const TracccScalar sinPhi = y * invSinTheta;
...
inline TracccMatrix<3, 3> cross(const TracccMatrix<3, 3>& m, const Vector3& v) {
  TracccMatrix<3, 3> r;
...
inline std::pair<double, double> incidentAngles(
    const traccc::Vector3& direction,
    const traccc::RotationMatrix3& globalToLocal) {
  traccc::Vector3 trfDir = globalToLocal * direction;
