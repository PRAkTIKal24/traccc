...
// clang-format off
/// @brief macro to use a local traccc::Logger object
/// @ingroup Logging
///
/// @param log_object logger instance of type
//         <tt>std::unique_ptr<const traccc::Logger></tt>
///
...
/// @code{.cpp}
/// void myFunction() {
///    std::unique_ptr<const traccc::Logger> myLogger
///        = /* .. your initialization .. */;
...
#define TRACCC_LOCAL_LOGGER(log_object)                                          \
  struct __local_traccc_logger                                                   \
  {                                                                            \
    __local_traccc_logger(std::unique_ptr<const ::traccc::Logger> logger):         \
      m_logger(std::move(logger))                                              \
    {}                                                                         \
                                                                               \
    const ::traccc::Logger& operator()() const                                   \
    {                                                                          \
      return *m_logger;                                                        \
    }                                                                          \
                                                                               \
  ...
  __local_traccc_logger logger(log_object);
...
///      macro is used and it must return a traccc::Logger object.
///
/// The debug message is printed if the current traccc::Logging::Level <=
/// traccc::Logging::VERBOSE.
...
///       and @c TRACCC_LOG_FAILURE_THRESHOLD, this operations is either constexpr
///       or a runtime operation.
Level getFailureThreshold();

#else

#ifdef TRACCC_ENABLE_LOG_FAILURE_THRESHOLD
#ifdef TRACCC_LOG_FAILURE_THRESHOLD
// We have a fixed compile time log failure threshold
constexpr Level getFailureThreshold() {
  return Level::TRACCC_LOG_FAILURE_THRESHOLD;
}
#else
Level getFailureThreshold();
#endif
...
/// @note This function is only available if @c TRACCC_LOG_FAILURE_THRESHOLD is
  ...
  explicit DefaultFilterPolicy(Level lvl) : m_level(lvl) {
    if (lvl > getFailureThreshold()) {
      throw ThresholdFailure(
          ...
          "the TRACCC_LOG_FAILURE_THRESHOLD=" +
