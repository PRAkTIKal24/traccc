...
#include "traccc/edm/detail/TransformationFreeToBound.hpp"
#include "traccc/Surfaces/Surface.hpp"
#include "traccc/utils/UnitVectors.hpp"
#include "traccc/utils/detail/periodic.hpp"
...
class GenericBoundTrackParameters {
 public:
  ...
  TracccVector<2> spatialImpactParameters() const { return m_params.head<2>(); }
  /// Vector of spatial and temporal impact parameters (i.e., d0, z0, and t)
  TracccVector<3> impactParameters() const {
    TracccVector<3> ip;
  ...
  std::optional<TracccSquareMatrix<2>> spatialImpactParameterCovariance() const {
  ...
  /// d0, z0, and t)
  std::optional<TracccSquareMatrix<3>> impactParameterCovariance() const {
    if (not m_cov.has_value()) {
      return std::nullopt;
    }

    TracccSquareMatrix<3> ipCov;
