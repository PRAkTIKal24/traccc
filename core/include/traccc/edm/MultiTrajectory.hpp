...
#include "traccc/edm/SourceLink.hpp"
#include "traccc/edm/TrackStatePropMask.hpp"
#include "traccc/edm/TrackStateProxyConcept.hpp"
#include "traccc/edm/TrackStateType.hpp"
#include "traccc/edm/Types.hpp"
#include "traccc/geometry/GeometryContext.hpp"
#include "traccc/utils/AlgebraHelpers.hpp"
#include "traccc/utils/Concepts.hpp"
#include "traccc/utils/HashedString.hpp"
#include "traccc/utils/Helpers.hpp"
#include "traccc/utils/ThrowAssert.hpp"
...
class TrackStateProxy {
 public:
  ...
  void shareFrom(const TrackStateProxy<Trajectory, M, ReadOnlyOther>& other,
    ...
    assert(TRACCC_CHECK_BIT(other.getMask(), shareSource) &&
  ...
  template <TRACCC_CONCEPT(TrackStateProxyConcept) track_state_proxy_t,
  ...
  void copyFrom(const track_state_proxy_t& other,
    ...
    if (onlyAllocated) {
      ...
      if (TRACCC_CHECK_BIT(src, PM::Calibrated)) {
      ...
      if (TRACCC_CHECK_BIT(src, PM::Predicted)) {
        predicted() = other.predicted();
        predictedCovariance() = other.predictedCovariance();
      }

      if (TRACCC_CHECK_BIT(src, PM::Filtered)) {
        filtered() = other.filtered();
        filteredCovariance() = other.filteredCovariance();
      }

      if (TRACCC_CHECK_BIT(src, PM::Smoothed)) {
      ...
      if (TRACCC_CHECK_BIT(src, PM::Jacobian)) {
      ...
      if (TRACCC_CHECK_BIT(mask, PM::Predicted) &&
      ...
      if (TRACCC_CHECK_BIT(mask, PM::Filtered) && has<hashString("filtered")>() &&
          other.template has<hashString("filtered")>()) {
        filtered() = other.filtered();
        filteredCovariance() = other.filteredCovariance();
      }

      if (TRACCC_CHECK_BIT(mask, PM::Smoothed) && has<hashString("smoothed")>() &&
      ...
      if (TRACCC_CHECK_BIT(mask, PM::Jacobian) && has<hashString("jacobian")>() &&
          other.template has<hashString("jacobian")>()) {
        jacobian() = other.jacobian();
      }

      if (TRACCC_CHECK_BIT(mask, PM::Calibrated) &&
  ...
  template <size_t kMeasurementSize, bool RO = ReadOnly,
            typename = std::enable_if_t<!RO>>
  void setCalibrated(
      const traccc::Measurement<BoundIndices, kMeasurementSize>& meas) {
 ...
 private:
  ...
  friend class traccc::MultiTrajectory<Trajectory>;
...
class MultiTrajectory {
 ...
 private:
  ...
  constexpr bool checkOptional(HashedString key, IndexType istate) const {
    using namespace traccc::HashedStringLiteral;
    switch (key) {
...
#include "traccc/edm/MultiTrajectory.ipp"
