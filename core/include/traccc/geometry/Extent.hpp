...
#include "traccc/utils/Enumerate.hpp"
#include "traccc/utils/Range1D.hpp"
#include "traccc/utils/RangeXD.hpp"
...
using Envelope = std::array<TracccScalar, 2>;
...
class Extent {
 public:
  ...
  void set(BinningValue bValue, TracccScalar min, TracccScalar max);
  ...
  Range1D<TracccScalar>& range(BinningValue bValue);
  ...
  const std::array<std::vector<TracccScalar>, binValues>& valueHistograms() const;

  /// Access the minimum parameter
  ///
  /// @param bValue the binning identification
  TracccScalar min(BinningValue bValue) const { return m_range[bValue].min(); }

  /// Access the maximum parameter
  ///
  /// @param bValue the binning identification
  TracccScalar max(BinningValue bValue) const { return m_range[bValue].max(); }

  /// Access the midpoint
  ///
  /// @param bValue the binning identification
  TracccScalar medium(BinningValue bValue) const {
  ...
  ///
  /// @param bValue the binning identification
  TracccScalar interval(BinningValue bValue) const {
 ...
 private:
  ...
  RangeXD<binValues, TracccScalar> m_range;
  /// A potential envelope
  ExtentEnvelope m_envelope = zeroEnvelopes;
  /// (Optional) Value histograms for bin detection
  std::array<std::vector<TracccScalar>, binValues> m_valueHistograms;
};

inline Range1D<TracccScalar>& traccc::Extent::range(BinningValue bValue) {
  return m_range[bValue];
}

inline const Range1D<TracccScalar>& traccc::Extent::range(
    BinningValue bValue) const {
  return m_range[bValue];
}

inline const RangeXD<binValues, TracccScalar> Extent::range() const {
  return m_range;
}
...
inline const std::array<std::vector<TracccScalar>, binValues>&
