...
struct GeometryIdentifierHook {
  ...
  virtual traccc::GeometryIdentifier decorateIdentifier(
      traccc::GeometryIdentifier identifier, const traccc::Surface& surface) const;
};
...
namespace std {
template <>
struct hash<traccc::GeometryIdentifier> {
  auto operator()(traccc::GeometryIdentifier gid) const noexcept {
    return std::hash<traccc::GeometryIdentifier::Value>()(gid.value());
  }
};
}  // namespace std
