...
#include "traccc/Surfaces/detail/VerticesHelper.hpp"
...
class BoundaryCheck {
 ...
 private:
  ...
  BoundaryCheck transformed(const TracccMatrix<2, 2>& jacobian) const;
...
inline traccc::BoundaryCheck::Type traccc::BoundaryCheck::type() const {
  return m_type;
}

inline const traccc::Vector2& traccc::BoundaryCheck::tolerance() const {
  return m_tolerance;
}

inline traccc::SquareMatrix2 traccc::BoundaryCheck::covariance() const {
  return m_weight.inverse();
}

inline traccc::BoundaryCheck::BoundaryCheck(bool check)
    : m_weight(SquareMatrix2::Identity()),
      m_tolerance(0, 0),
      m_type(check ? Type::eAbsolute : Type::eNone) {}

inline traccc::BoundaryCheck::BoundaryCheck(bool checkLocal0, bool checkLocal1,
...
inline traccc::BoundaryCheck::BoundaryCheck(const SquareMatrix2& localCovariance,
                                          double sigmaMax)
    : m_weight(localCovariance.inverse()),
      m_tolerance(sigmaMax, 0),
      m_type(Type::eChi2) {}

inline traccc::BoundaryCheck traccc::BoundaryCheck::transformed(
    const TracccMatrix<2, 2>& jacobian) const {
...
template <typename Vector2Container>
inline bool traccc::BoundaryCheck::isInside(
...
inline bool traccc::BoundaryCheck::isInside(const Vector2& point,
...
template <typename Vector2Container>
inline double traccc::BoundaryCheck::distance(
    const traccc::Vector2& point, const Vector2Container& vertices) const {
...
inline double traccc::BoundaryCheck::distance(const traccc::Vector2& point,
                                            const Vector2& lowerLeft,
                                            const Vector2& upperRight) const {
  if (m_type == Type::eNone || m_type == Type::eAbsolute) {
...
inline bool traccc::BoundaryCheck::isTolerated(const Vector2& delta) const {
  if (m_type == Type::eNone) {
    return true;
...
inline double traccc::BoundaryCheck::squaredNorm(const Vector2& x) const {
  return (x.transpose() * m_weight * x).value();
}

template <typename Vector2Container>
inline traccc::Vector2 traccc::BoundaryCheck::computeClosestPointOnPolygon(
    const traccc::Vector2& point, const Vector2Container& vertices) const {
...
inline traccc::Vector2
traccc::BoundaryCheck::computeEuclideanClosestPointOnRectangle(
    const Vector2& point, const Vector2& lowerLeft,
    const Vector2& upperRight) const {
  /*
   *
