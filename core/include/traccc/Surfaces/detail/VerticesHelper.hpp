...
/// @return a vector
std::vector<TracccScalar> phiSegments(TracccScalar phiMin = -M_PI,
                                    TracccScalar phiMax = M_PI,
                                    const std::vector<TracccScalar>& phiRefs = {},
                                    TracccScalar phiTolerance = 1e-6);
...
void createSegment(std::vector<vertex_t>& vertices,
                   std::pair<TracccScalar, TracccScalar> rxy, TracccScalar phi1,
                   TracccScalar phi2, unsigned int lseg, int addon = 0,
  ...
  segs = segs > 0 ? segs : 1;
  TracccScalar phistep = (phi2 - phi1) / segs;
  // Create the segments
  for (unsigned int iphi = 0; iphi < segs + addon; ++iphi) {
    TracccScalar phi = phi1 + iphi * phistep;
...
std::vector<Vector2> ellipsoidVertices(TracccScalar innerRx, TracccScalar innerRy,
                                       TracccScalar outerRx, TracccScalar outerRy,
                                       TracccScalar avgPhi = 0.,
                                       TracccScalar halfPhi = M_PI,
                                       unsigned int lseg = 1);
...
/// @param lseg The number of segments for for a full 2*pi segment
/// @return a vector of 2d-vectors
std::vector<Vector2> circularVertices(TracccScalar innerR, TracccScalar outerR,
                                      TracccScalar avgPhi = 0.,
                                      TracccScalar halfPhi = M_PI,
                                      unsigned int lseg = 1);
